...
// ---- Auto-refresh when tabs change (background.js sends this message) ----
let _autoRefreshTimer = null;
chrome.runtime.onMessage.addListener((msg) => {
  if (msg.type !== 'tabs-changed') return;
  clearTimeout(_autoRefreshTimer);
  _autoRefreshTimer = setTimeout(() => renderDashboard(), 300);
});
...
async function focusTab(url) {
  ...
  // Prefer the tab in the current window; fall back to another window
    ...
    <div class="deferred-item" data-deferred-id="${item.id}">
      ...
      <div class="deferred-info">
        <a href="${item.url}" target="_blank" rel="noopener" class="deferred-title" title="${(item.title || '').replace(/"/g, '&quot;')}">
          <img src="${faviconUrl}" alt="" style="width:14px;height:14px;vertical-align:-2px;margin-right:4px" onerror="this.style.display='none'">${escHtml(item.title || item.url)}
        </a>
...
async function renderStaticDashboard() {
  ...
  for (const tab of realTabs) {
    try {
      if (isLandingPage(tab.url, allSites)) {
  ...
  const landingHostnames = new Set(allSites.map(p => p.hostname).filter(Boolean));
  const landingSuffixes  = allSites.map(p => p.hostnameEndsWith).filter(Boolean);
  ...
  if (domainGroups.length > 0 && openTabsSection) {
    ...
    if (openTabsSectionCount) openTabsSectionCount.innerHTML = `${domainGroups.length} domain${domainGroups.length !== 1 ? 's' : ''} &nbsp;&middot;&nbsp; <button class="action-btn close-tabs" data-action="close-all-open-tabs" style="font-size:11px;padding:3px 10px;">${ICONS.close} Close all ${realTabs.length} tabs</button>`;
    openTabsSection.style.display = 'block';
    diffMissions(openTabsMissionsEl, domainGroups);
    const _searchInput = document.getElementById('searchInput');
    if (_searchInput?.value) filterTabs(_searchInput.value);
...
async function renderSettingsPanel() {
  ...
  const s = await getSettings();
  const savedHostnames = new Set(s.homepageSites.map(r => r.hostname));

  // Build quick-add chips from open tabs not already in the list
  const seen = new Set();
  const quickAdd = [];
  for (const tab of openTabs) {
    try {
      const { hostname } = new URL(tab.url || '');
      if (!hostname || savedHostnames.has(hostname) || seen.has(hostname)) continue;
      if ((tab.url || '').startsWith('chrome') || (tab.url || '').startsWith('about:')) continue;
      seen.add(hostname);
      quickAdd.push({ hostname, label: friendlyDomain(hostname) });
    } catch {}
  }
    ...
    <div class="settings-quick-add">
      ${quickAdd.map(q => `
        <button class="settings-quick-chip" data-action="quick-add-site"
          data-hostname="${escHtml(q.hostname)}" data-label="${escHtml(q.label)}">
          + ${escHtml(q.label)}
        </button>`).join('')}
    </div>
  ` : '';

  const sitesHtml = s.homepageSites.length > 0
    ? s.homepageSites.map(site => `
      <div class="settings-site-row">
...
document.addEventListener('click', async (e) => {
  ...
  // ---- Quick-add a site from current tabs ----
  if (action === 'quick-add-site') {
    const hostname = actionEl.dataset.hostname;
    const label    = actionEl.dataset.label || friendlyDomain(hostname);
    if (!hostname) return;
  ...
  // ---- Remove a site from Homepages ----
  if (action === 'remove-homepage-site') {
    const hostname = actionEl.dataset.hostname;
    if (!hostname) return;
